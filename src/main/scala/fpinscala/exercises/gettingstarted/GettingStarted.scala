...
object MyProgram:
  ...
  def fibonacciFrom(a: Int, b: Int): LazyList[Int] =
    a #:: fibonacciFrom(b, a + b)

  def fib(n: Int): Int = fibonacciFrom(0, 1).apply(n)
...
object PolymorphicFunctions:
  ...
  def uncurry[A, B, C](f: A => B => C): (A, B) => C = {
    // (a: A, b: B) => f(a)(b) // FS: My original answer
    (a, b) => f(a)(b)
  }
  ...
  def compose[A, B, C](f: B => C, g: A => B): A => C = {
    // a => f(g(a)) // FS: My original answer
    // g andThen f // Option 1
    f compose g // Option 2
  }
